...
def fillLegend(data, backgrounds, signals, stat):
    ...
    # signals
    leg.AddEntry(signals['ggH125'], 'ggh SM Higgs(125)x50', 'l')
    leg.AddEntry(signals['VBF125'], 'VBF SM Higgs(125)x50', 'l')
...
def BuildPlot(args):
    ...
    # We can skip this because it only has one variable
    ...
    sig_yields = [ihist.GetMaximum() for ihist in [signals['ggH125'], signals['VBF125']]] + \
