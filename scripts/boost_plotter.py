...
def fillLegend(data, backgrounds, signals, stat):
    ...
    # signals
    leg.AddEntry(signals['ggh125_powheg'], 'ggH Higgs(125)x50', 'l')

    leg.AddEntry(signals['vbf125_powheg'], 'VBF Higgs(125)x50', 'l')
...
def BuildPlot(args):
    ...
    for hkey in variable.GetListOfKeys():
        ...
        elif hname in plot_tools.boost_style_map['signals']:
            ihist = plot_tools.ApplyStyle(ihist, plot_tools.boost_style_map['signals'][hname])
            signals[hname] = ihist
    ...
    data_hist = plot_tools.ApplyStyle(data_hist, plot_tools.boost_style_map['data_obs'])
